...
#pragma warning restore 414
        ...
        [
            Tooltip(
                "Selective = to specifically bring objects to glow, Fullscreen = complete screen glows")
        ]
        ...
        [
            Tooltip(
                "Downsampling steps of the blur. Higher samples = better performance, but gains more flickering")
        ]
...
#region GET_SET
        ...
        {
            if (!blurShader) blurShader = Shader.Find("Hidden/MK/Glow/Blur");

            if (!compositeShader)
                compositeShader = Shader.Find("Hidden/MK/Glow/Composite");

            if (!selectiveRenderShader)
                selectiveRenderShader =
                    Shader.Find("Hidden/MK/Glow/SelectiveRender");
        }
        ...
        RenderTexture
        GetTemporaryRT(int width, int height, VRTextureUsage vrUsage)
        {
        ...
        private void Blur(RenderTexture main, RenderTexture tmpMain)
        {
            for (int i = 1; i <= blurIterations; i++)
            {
                float offsetInner =
                    i *
                    (blurSpreadInner * BLUR_SPREAD_INNTER_MULT) /
                    blurIterations /
                    samples;
        ...
        private GameObject SelectiveGlowCameraObject
        {
            get
            {
                ...
                {
                    selectiveGlowCameraObject =
                        new GameObject("selectiveGlowCameraObject");
                    selectiveGlowCameraObject.AddComponent<Camera>();
                    selectiveGlowCameraObject.hideFlags =
        ...
        private Camera SelectiveGlowCamera
        {
            get
            {
                ...
                {
                    selectiveGlowCamera =
                        SelectiveGlowCameraObject.GetComponent<Camera>();
                }
        ...
        private void FullScreenGlow(
            RenderTexture src,
            RenderTexture dest,
            RenderTexture glowTexInner,
        ...
        private void OnPostRender()
        {
            switch (glowType)
            {
                case GlowType.Selective:
                    ...
                    glowTexRaw =
                        RenderTexture
                            .GetTemporary((int)((Cam.pixelWidth) / samples),
                            (int)((Cam.pixelHeight) / samples),
                            16,
                            rtFormat,
                            RenderTextureReadWrite.Default,
                            1,
                    ...
                    SetupGlowCamera();
                    SelectiveGlowCamera
                        .RenderWithShader(selectiveRenderShader, "RenderType");
                    break;
            ...
            compositeMaterial
                .SetFloat("_GlowIntensityInner",
                glowIntensityInner *
                (
                (glowType != GlowType.Fullscreen)
                    ? GLOW_INTENSITY_MULT * blurSpreadInner
                    : 10.0f
                ));
        ...
        private void OnRenderImage(RenderTexture src, RenderTexture dest)
        {
            ...
            RenderTexture glowTexInner =
                GetTemporaryRT(srcWidth, srcHeight, src.vrUsage);
            RenderTexture tmpGlowTex =
                GetTemporaryRT(srcWidth, srcHeight, src.vrUsage);
#else
